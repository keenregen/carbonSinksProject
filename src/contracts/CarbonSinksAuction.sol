...
contract CarbonSinksAuction is ERC721URIStorage, ReentrancyGuard {
    ...
    constructor(uint _royaltyFee) ERC721("Carbon Sink", "SINK") {
    ...
    function makeAuction(
        ...
        require(price > 0 ether, "Sales price must be greater than 0.");
        require(
        ...
        emit AuctionItemReady(tokenId, msg.sender, address(0), price, false);
    }
    ...
    function buyAuctionedItem(uint tokenId) public payable nonReentrant {
        ...
        require(!auctionedItem[tokenId].biddable, "The Auction is only for purchase");
        ...
        payTo(auctionedItem[tokenId].owner, (msg.value - royalty));
        payTo(seller, royalty);
        IERC721(address(this)).transferFrom(
            address(this),
            msg.sender,
